...
Phase 10（コマンドメニューUI・移動取り消し）まで Play 確認済み・コミット済み。
次は Phase 11（救出システム）。着手前にプランファイルの確認事項 (a)〜(h) を作者と合意すること。
...
- Core/ … BattleController(操作の司令塔・4状態FSM: Idle/MoveSelect/CommandMenu/TargetSelect),
  ActionContext(1行動の文脈・移動取り消し), TurnManager(フェイズ・勝敗・簡易UI), BattleSetup(StageDataを読んで初期配置)
...
- Phase 11: 救出システム ← 次はここ。着手前にプランの確認事項 (a)〜(h)（運び手死亡時の貨物、
  降ろされたユニットの行動状態など）を作者と合意してから実装する → Phase 12: 輸送隊
