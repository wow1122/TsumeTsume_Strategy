...
Phase 19（ガードの活用: 挟撃されそうな味方への守り配置・ThreatMap 新設）まで
Play 確認済み・コミット済み。Phase 8〜15 ロードマップは完走。
現在は敵AI強化ロードマップ（Phase 16〜21・2026-07-16 作者承認）を進行中。次は Phase 20（飛翔の活用）。
...
- AI/ … EnemyAI(点数評価型・「考えるEvaluate→動かすExecute」の2段構え・
  思考の順番は 攻撃→待ち伏せゲート→ガード→接近※Phase 19・
  接近は距離マップの3段構え(経路→敵無視→直線距離)・日本語思考ログ・同点は地形防御の高いマス優先・
  性格ゲート※待ち伏せ型は挑発まで動かない＝挑発判定はTakeAction冒頭(被弾)と攻撃プラン成立時・
  ...
  ActionPlan(行動予定の構造体+ActionKind。Guardは実装済み※Phase 19、
  TakeOff/LandAndWaitはPhase 20用の予約)
...
（Unit_Player, Unit_Enemy, Unit_Player_Archer, Unit_Player_Mage, Unit_Player_Cavalry,
  ...
  Stage_AITest_Ambush, Stage_AITest_Pincer, Stage_AITest_Guard, TerrainTable）
シーン: Assets/_Game/Scenes/BattleScene.unity

## 次にやること

敵AI強化ロードマップ（Phase 16〜21）進行中。Phase 16 完了（271755e）・Phase 17 完了（f4c4b61）・
Phase 18 完了（bceef88）・Phase 19 完了（6746206）。
範囲の合意（2026-07-16・選択式ダイアログで確認済み）: 経路ベース接近＋敵の性格付け＋集中攻撃＋
確定枠（挟撃→ガード→飛翔の順）。ガードは攻撃優先。救出系AIと勝利条件バリエーションは対象外。
- **Phase 20: 飛翔の活用（離陸・着陸判断）** — 着手は作者の明示の合図を待つ
  （プラン承認だけを開始合図にしない。詳細は下の「運用の注意」参照）。
  キックオフではロードマップの Phase 20 節を確認する: 離陸条件・空中巡航（対空射程の回避）・
  着陸の損得（残り2ターン以上なら手動着陸で1ターン得）・probe方式（try/finallyで必ず復元）・
  アセット編集の同意（Stage_01(3,9) と Stage_Test(5,8) の initialFlightTurns 99→0、
  Stage_AITest_Flight 新規）
- 以降: Phase 21 集中攻撃＋統合調整
