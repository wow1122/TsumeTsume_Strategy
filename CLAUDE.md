...
Phase 16（敵AI土台: 経路ベースの接近・評価/実行の分離・思考ログ・検証ステージ）まで
Play 確認済み・コミット済み。Phase 8〜15 ロードマップは完走。
現在は敵AI強化ロードマップ（Phase 16〜21・2026-07-16 作者承認）を進行中。次は Phase 17（敵の性格付け）。

全体計画（Phase 8〜15 ロードマップ）は
`/home/wawo/.claude/plans/tsumetsume-strategy-phase8-tsumetsume-st-elegant-parasol.md` 参照（WSL側）。
敵AI強化ロードマップ（Phase 16〜21・フェーズ詳細と各キックオフ合意項目）は
`/home/wawo/.claude/plans/tsumetsume-strategy-ai-ai-ai-robust-quasar.md` 参照（WSL側）。
...
- AI/ … EnemyAI(点数評価型・「考えるEvaluate→動かすExecute」の2段構え・
  ...
  ActionPlan(行動予定の構造体+ActionKind。Guard/TakeOff/LandAndWaitは後のフェーズ用の予約)
...
（Unit_Player, Unit_Enemy, Unit_Player_Archer, Unit_Player_Mage, Unit_Player_Cavalry,
  ...
  Unit_Enemy_AxeCavalry, Unit_Test_Dummy,
  W_Sword, W_Axe, W_Lance, W_Bow, W_Tome, Stage_Test, Stage_01, Stage_AITest_Path, TerrainTable）
シーン: Assets/_Game/Scenes/BattleScene.unity

## 次にやること

敵AI強化ロードマップ（Phase 16〜21）進行中。Phase 16 完了・コミット済み（271755e）。
範囲の合意（2026-07-16・選択式ダイアログで確認済み）: 経路ベース接近＋敵の性格付け＋集中攻撃＋
確定枠（挟撃→ガード→飛翔の順）。ガードは攻撃優先。救出系AIと勝利条件バリエーションは対象外。
- **Phase 17: 敵の性格付け（突撃型/待ち伏せ型）** — 着手は作者の明示の合図を待つ
  （プラン承認だけを開始合図にしない。詳細は下の「運用の注意」参照）。
  キックオフ合意項目: (a)挑発の定義（被弾 or 今すぐ攻撃できる。連鎖挑発はv1なし）
  (b)Stage_01 のどの配置を待ち伏せ型にするか（案: 城の守備隊） (c)未挑発は一切動かない
  （立ち位置による受動ガードは効く）の確認 (d)アセット編集の同意
- 以降: Phase 18 挟撃の活用 → 19 ガードの活用 → 20 飛翔の活用 → 21 集中攻撃＋統合調整
  （各フェーズの詳細・合意項目はロードマップのプランファイル参照）
- Stage_01 のバランス調整（作者のプレイ感想を聞いて数値・配置をアセット編集で反映）は並行して随時
- 運用の注意: 選択式ダイアログの回答は履歴で見つけにくい。回答をもらったら次の返信で必ず復唱し、
