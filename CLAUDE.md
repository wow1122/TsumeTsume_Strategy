...
続けて**所持品（インベントリ）ロードマップ（Phase 22〜25）に着手**（2026-07-19 キックオフ合意。「次にやること」の節参照）。
フェーズ22（所持品データ基盤・挙動不変）はコミット済み（3293343）、
フェーズ23（装備コマンド）もコミット済み（feat d8f5d04 / data cb6effb）。
...
現在は**所持品（インベントリ）ロードマップ（Phase 22〜25）を進行中**
（作者と 2026-07-19 にキックオフ合意。最優先で着手で合意済み）。

### 所持品（インベントリ）ロードマップ（Phase 22〜25・2026-07-19 キックオフ合意・進行中）
...
- 各フェーズの内容と現状（feat スクリプト + data アセットの小刻みコミット）:
  - **フェーズ22（所持品データ基盤・挙動不変）＝コミット済み（3293343。Class_MasterThief.asset の
    再直列化差分は chore 7c7331b）**。新規 ItemData/ToolData/ItemSlot/ToolDataEditor、WeaponData の基底差し替え、
    UnitData に items+InventoryCapacity、Unit に EquippedWeapon と Weapon 窓口差し替え、17体に items（既存 weapon と同じ武器1件）を追記
  - **フェーズ23（装備コマンド）＝Play 確認済み・コミット済み（feat d8f5d04 / data cb6effb・2026-07-19）**。ItemListMenu.cs 新規、
    BattleController.cs・Unit.cs 変更（Equip/Unequip）、味方騎兵 Unit_Player_Cavalry を [剣, 槍] に。
    BuildCommands の「所持品」は「代わりに降ろす」の後・「待機」の前（救出/引き受けの制限分岐では出さない）。
    装備・武装解除は行動消費なしで実行後にメニューを開き直す。行は 武器（装備可/装備不可/装備中）・道具・最下行「武装解除」
  - フェーズ24（道具・傷薬）＝未着手。Tool_Vulnerary.asset（HP10回復・3回）、Unit.Heal/UseTool、
    道具行→使用→行動終了、HP満タンは無効行。味方兵・味方騎兵に傷薬1個ずつ配布
  - フェーズ25（杖「ライブ」）＝未着手。Combat/StaffRules.cs・W_Live.asset（杖・後衛・威力10・射程1）・
    Unit_Player_Monk.asset（味方修道士・兵種 Class_Monk〔歩兵・移動3・杖のみ〕・items [ライブ, 傷薬]）。
    杖コマンド→UnitTargetSelect(Heal)→回復（魔力+威力）→行動終了、緑の回復範囲表示、
    対空判定から杖を除外（CombatRules.IsAntiAirCapable を新設し CanEngage・すり抜け・対空圏の3箇所を差し替え）、
    Stage_01 (7,0) に修道士を配置（味方11体目）
- プラン全文（作者の編集用コピー）: `C:\Users\wawo\Desktop\所持品ロードマップ（フェーズ22〜25）実装プラン.md`

### 所持品ロードマップの後の持ち越し候補（優先度は作者が決める）

過去の合意で先送りになった項目のメモ。所持品ロードマップの後に作者と優先度を相談する:
...
- 兵種データの残り（クラスチェンジの実行・特攻ダメージの戦闘反映・光魔法の効果・主人公＝軍師ユニットの作成。
  データ基盤は導入済み。所持品ロードマップで特効武器・杖効果の土台が入るので一部は前進しやすくなる）
