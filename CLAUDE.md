...
Phase 17（敵の性格付け: 突撃型/待ち伏せ型・挑発システム）まで
Play 確認済み・コミット済み。Phase 8〜15 ロードマップは完走。
現在は敵AI強化ロードマップ（Phase 16〜21・2026-07-16 作者承認）を進行中。次は Phase 18（挟撃の活用）。
...
（Unit_Player, Unit_Enemy, Unit_Player_Archer, Unit_Player_Mage, Unit_Player_Cavalry,
  ...
  Stage_AITest_Ambush, TerrainTable）
...
- **Phase 18: 挟撃の活用（お膳立て）** — 着手は作者の明示の合図を待つ
  （プラン承認だけを開始合図にしない。詳細は下の「運用の注意」参照）。
  キックオフ合意項目: (a)お膳立て加点は控えめ +2（実ダメージ差1点までしか覆さない）の方針確認
  (b)任意項目「ガード役潰し」（一撃で倒せる相手が複数なら、誰かをガード中の相手を優先 +50）を
  入れるかどうか (c)挟撃判定の抽出リファクタ（GetPincerOppositeCell / WouldGuardAt。挙動不変）
  (d)検証用ステージ Stage_AITest_Pincer 作成の同意
- 以降: Phase 19 ガードの活用 → 20 飛翔の活用 → 21 集中攻撃＋統合調整
