...
Phase 20（飛翔の活用: 敵飛行兵の離陸・巡航・着陸判断・probe方式）まで
Play 確認済み・コミット済み。Phase 8〜15 ロードマップは完走。
現在は敵AI強化ロードマップ（Phase 16〜21・2026-07-16 作者承認）を進行中。次は Phase 21（集中攻撃＋統合調整）。
...
（Unit_Player, Unit_Enemy, Unit_Player_Archer, Unit_Player_Mage, Unit_Player_Cavalry,
  ...
  Stage_AITest_Ambush, Stage_AITest_Pincer, Stage_AITest_Guard, Stage_AITest_Flight, TerrainTable）
シーン: Assets/_Game/Scenes/BattleScene.unity

## 次にやること

敵AI強化ロードマップ（Phase 16〜21）進行中。Phase 16 完了（271755e）・Phase 17 完了（f4c4b61）・
Phase 18 完了（bceef88）・Phase 19 完了（6746206）・Phase 20 完了（4c3f415）。
範囲の合意（2026-07-16・選択式ダイアログで確認済み）: 経路ベース接近＋敵の性格付け＋集中攻撃＋
確定枠（挟撃→ガード→飛翔の順）。ガードは攻撃優先。救出系AIと勝利条件バリエーションは対象外。
- **Phase 21: 集中攻撃＋統合調整（ロードマップ最終フェーズ）** — 着手は作者の明示の合図を待つ
  （プラン承認だけを開始合図にしない。詳細は下の「運用の注意」参照）。
  キックオフではロードマップの Phase 21 節を確認する: EnemyAI.BeginEnemyPhase（敵フェイズ開始の
  前処理で「皆で倒し切れる最小HPの相手」を focusTarget に・FocusBonus +200・倒れたら再選定・
  静的状態は RuntimeInitializeOnLoadMethod でリセット）・Stage_01 通しプレイ×2 での
  バランス調整（点数定数とアセットのみで調整）・CLAUDE.md のAI節の全面更新
- ロードマップ完走後: 次期ロードマップを作者と相談（勝利条件バリエーション・拠点到達が持ち越し候補）
