...
Phase 18（挟撃の活用: お膳立て加点・ガード役潰し・挟撃判定の部品化）まで
Play 確認済み・コミット済み。Phase 8〜15 ロードマップは完走。
現在は敵AI強化ロードマップ（Phase 16〜21・2026-07-16 作者承認）を進行中。次は Phase 19（ガードの活用）。
...
- AI/ … EnemyAI(点数評価型・「考えるEvaluate→動かすExecute」の2段構え・
  ...
  挟撃のお膳立て+2とガード役潰し+50※Phase 18。点数定数は先頭の1ブロックに集約),
...
（Unit_Player, Unit_Enemy, Unit_Player_Archer, Unit_Player_Mage, Unit_Player_Cavalry,
  ...
  Stage_AITest_Ambush, Stage_AITest_Pincer, TerrainTable）
...
- **Phase 19: ガードの活用（守り配置）** — 着手は作者の明示の合図を待つ
  （プラン承認だけを開始合図にしない。詳細は下の「運用の注意」参照）。
  キックオフ合意項目: (a)攻撃優先の確認（守り位置を離れて攻撃に行くこともある。ログで分かる）
  (b)ガード＞接近の規則（防げるダメージが正なら接近よりガード。同点は防止量→地形防御→移動量）
  (c)未挑発の待ち伏せ型はガード移動しない（受動ガードは効く）
  (d)検証用ステージ Stage_AITest_Guard 作成の同意
- 以降: Phase 20 飛翔の活用 → 21 集中攻撃＋統合調整
