...
フェーズ23（装備コマンド）は作業ツリーで実装中（未コミット）。
...
- Core/ … BattleController(操作の司令塔・8状態FSM: Idle/MoveSelect/CommandMenu/TargetSelect/UnitTargetSelect/TileSelect/CargoSelect/DamageCheck。
...
- 各フェーズの内容と現状（feat スクリプト + data アセットの小刻みコミット）:
  ...
  - **フェーズ23（装備コマンド）＝作業ツリーで実装中（未コミット・Play 未確認）**。ItemListMenu.cs 新規、
